...
- **Workers AI**：`@cf/openai/gpt-oss-120b` 進行挑戰結算（`src/llm.ts`）。
...
⚠️ **本地開發的 AI 限制**：`wrangler dev`（純 local）不支援 AI 綁定，挑戰一律走「不計分」fallback，適合測試對戰流程。要實測 AI 評分需登入 Cloudflare 後使用：
