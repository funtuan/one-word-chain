...
export async function judge(
  ...
  lines.push(
    "全部以台灣日常用法判斷。",
    "【評語 reason 的口吻】評分本身照上述標準嚴謹判定，但 reason 要用「毒舌吐槽」的口吻寫：像綜藝節目的嘴賤評審，一針見血、好笑但不帶髒字，用台灣口語。評語仍要讓玩家看得出這個字為什麼加分或扣分，不能只罵不講理由。繁體中文、30 字以內。",
    "口吻範例（只學語氣，別照抄）：接得好→「這個字接得漂亮，我竟然挑不出毛病，可惡」；平淡→「能用，但無聊到我打了個哈欠」；湊字→「這個字存在感跟空氣一樣，拿掉根本沒人發現」；荒謬→「太陽在海裡游泳？你的物理老師在哭」。",
